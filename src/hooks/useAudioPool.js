...
export default function useAudioPool({ okSrc = "/sfx/drop.mp3", missSrc = "/sfx/wrong.wav", volume = 0.7 }) {
  ...
  useEffect(() => {
    ...
    const ok1 = new Audio(okSrc), ok2 = new Audio(okSrc);
    const m1 = new Audio(missSrc), m2 = new Audio(missSrc);
    const all = [ok1, ok2, m1, m2];
    all.forEach((a) => { a.preload = "auto"; a.volume = volume; });
  ...
  const unlockOnce = useCallback(() => {
    ...
    a.muted = true;
    a.play().then(() => {
      a.pause(); a.currentTime = 0; a.muted = false; setAudioUnlocked(true);
    }).catch(() => {});
  }, [audioUnlocked]);

  const playFrom = useCallback((poolRef) => {
